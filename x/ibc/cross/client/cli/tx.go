...
func GetCreateMsgInitiateCmd(cdc *codec.Codec) *cobra.Command {
	const (
		flagCrossChainTransaction = "contract"
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var txs []types.CrossChainTransaction
			txPaths := viper.GetStringSlice(flagCrossChainTransaction)
			...
			for i, path := range txPaths {
				...
				tx := types.CrossChainTransaction{
	...
	cmd.Flags().StringSlice(flagCrossChainTransaction, nil, "Save a result to this file")
	cmd.Flags().StringSlice(flagSourceChannel, nil, `"[channel]:[port]"`)
	cmd.MarkFlagRequired(flagCrossChainTransaction)
