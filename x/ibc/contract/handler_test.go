...
func (suite *HandlerTestSuite) TestHandleContractCall() {
	contractHandler := contract.NewContractHandler(suite.app.ContractKeeper, func(kvs sdk.KVStore, tp cross.StateConditionType) cross.State {
	...
	{
		{
			msg := contract.NewMsgContractCall(acc0, []sdk.AccAddress{acc0}, nil, cross.NewStateCondition(cross.ExactStateCondition, nil))
...

		{
			...
			msg := contract.NewMsgContractCall(acc0, []sdk.AccAddress{acc0}, bz, cross.NewStateCondition(cross.ExactStateCondition, nil))
	...
	// Ensure that commit store state successfully
	{
...

		{
			...
			bz, err := contract.EncodeContractSignature(c)
			suite.Require().NoError(err)
			msg := contract.NewMsgContractCall(acc1, []sdk.AccAddress{acc0}, bz, cross.NewStateCondition(cross.ExactStateCondition, nil))
			ctx, _ := suite.ctx.CacheContext()
